...
def readGeneralInput():
    ...
    G.initializingFlag = general.get(
        "initializingFlag", False
    )
    G.initializingFilename = general.get(
...
def createWIP():
    ...
    inputWIP = json_data.get("input", {})
    bom = inputWIP.get("BOM", None)
    if bom:
    ...
    if G.initializingFlag and G.initializingFilename != "":       # if in the input json file there is the flag to include initializeWIP call the function routeWIP
        initializeWIP()
...
# ===========================================================================
def initializeWIP():
    from manpy.simulation.imports import Entity
    # open the route file
    ...
    entityType = Globals.getClassFromName(entityClass)
    for i in range(len(initializingList)):                   # for every element in the initializeWIP file
        ...
        initializingQueueName = Globals.findObjectById("Q" + str(int(initializingList[i])))     # object in which the part is placed
        G.EntityList.append(entityR)                         # add the entity to the entity list
...
def main():
    ...
    filename_gen = input("Give the path to the JSON file:\n")
    filename_IN = path_IN + filename_gen
    filename_OUT = path_OUT + filename_gen[:-5] + "_Output.json"        # the output file name is equal to the input name + _Output
    # filename_OUT = input("Select the name of the output file:\n")     # if we want to select the output file name
    try:  # try to open the file with the inputs
        G.JSONFile = open(filename_IN, "r")  # global variable holding the file to be opened
    except IOError:
        print(("%s could not be open" % filename_IN))
        return "ERROR"
    G.InputData = (
        G.JSONFile.read()
    ...
    G.outputJSONFile = open(filename_OUT, mode="w")
