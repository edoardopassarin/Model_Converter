...
class ModelConverter:
    ...
    def __init__(self, graph_model, timed_config=None):
        ...
        self.timed_config = timed_config    # dictionary with additional information for the conversion

        self.unloadTime = None              # unload time for all the machine objects
        self.transportationTime = None      # transportation time for the eventual conveyors
        self.transportationCap = 0          # capacity of the conveyors
        self.loop_start = None              # machine that starts the loop for closed models
        self.queue_cap_config = None        # dictionary with values to override the capacity of the queues
    ...
    def convertModel(self):
        ...
        # the "general" key is redundant, since the values are specified when the simulation is run
        ...
        for q in range(self.numOfQueues):  # cycle for writing the queues with all their parameters
            self.outModel["graph"]["node"].update({
                "Q" + str(q + 1): {  # ID of the single queue
                    ...
                    "name": "Queue",
                    "wip": [],
                    "freq_parallel": self.arcs[q]['frequency']
                }
            })

        if self.transportationTime:
