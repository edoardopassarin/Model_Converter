...
class CoreObject(ManPyObject):
    ...
    def __init__(self, id, name, **kw):
        ...
        self.SettingUp = []
        self.OnBreak = []
        self.jobsDone = []          # Added a list for the jobs completed on each replication
        ...
        self.gatherSysTime = False
        self.gatherIntArr = False
        ...
        # Flag to make this a priority giver (for case study)
        self.priority_giver = False

        # Attribute for the frequency in parallel
        self.freq_parallel = 1
        ...
        self.WipStat = []
        # keep systime stat for every replication
        self.SysTime = []
        # keep interarrival stat for every replication
        self.IntArr = []

        self.fromListFlag = False               # flag if machine processing times are from a list
    ...
    def initialize(self, env=None):
        ...
        # initialize the lists for the trace of WIP, systime and interarrival time
        self.SysTimeList = np.array([[0, 0, 0]])
        self.IntArrList = np.array([[0, 0, 0]])
    ...
    def getEntity(self):
        ...
        # calculate the interarrival time list
        if self.gatherIntArr:
            import numpy
            self.IntArrTemp = round(self.env.now - self.timeLastEntityEntered, 2)     # calculate the inter-arrival time
            if self.completedJobs == 0:
                import numpy as np
                self.IntArrList = np.array([[self.env.now, self.completedJobs+1, self.IntArrTemp]])
            else:
                self.IntArrList = numpy.concatenate(
                    (self.IntArrList, [[self.env.now, self.completedJobs + 1, self.IntArrTemp]])
                )
        ...
        if self.gatherSysTime:
            import numpy
            self.activeLifespan = self.env.now - activeEntity.startTime
            if self.completedJobs == 0:
                import numpy as np
                self.SysTimeList = np.array([[self.env.now, self.completedJobs+1, self.activeLifespan]])
            else:
                self.SysTimeList = numpy.concatenate(
                    (self.SysTimeList, [[self.env.now, self.completedJobs+1, self.activeLifespan]])
                )
    ...
    def signalReceiver(self, transmitter=None):
        ...
        if possibleReceivers:
            receiver = self.selectReceiver(self.env, possibleReceivers)
            receiversGiver = self
    ...
    # =======================================================================
    @staticmethod
    def selectReceiver(env=None, possibleReceivers=[]):
        ...
        total_freq = sum(obj.freq_parallel for obj in candidates)
        freq_temp = 0
        test_par = random()
        receiver = None
        from .Globals import G
        # Loop to find the receiver object basing on the frequency
        if candidates[0].name == 'Machine' or candidates[0].name == "Queue":
            for obj in candidates:
                perc_parallel = obj.freq_parallel/total_freq
                freq_temp += perc_parallel
                if test_par <= freq_temp:
                    receiver = obj
                    break
            ...
            for object in candidates:
                if env:
                    timeWaiting = (
                        env.now - object.timeLastEntityLeft
                    )
                else:
                    timeWaiting = (
    ...
    def signalGiver(self):
        ...
        if possibleGivers:
            giver = self.selectGiver(self.env, possibleGivers)
            giversReceiver = self
            # perform the checks that canAcceptAndIsRequested used to perform and update activeCallersList or assignExit and operatorPool
    ...
    # =======================================================================
    @staticmethod
    def selectGiver(env=None, possibleGivers=[]):
        candidates = possibleGivers
        # dummy variables that help prioritize the objects requesting to give objects to the object (activeObject)
        # it selects the giver prioritizing the ones that are waiting for the longest time
        ...
        for object in candidates:
            ...
            if env:
                timeWaiting = env.now - object.timeLastEntityEnded
            else:
                timeWaiting = G.env.now - object.timeLastEntityEnded
            if timeWaiting >= maxTimeWaiting:       # since it has an equal we select the last in the for cycle if they are wainting for the same amount of time
                giver = object  # the object to deliver the Entity to the activeObject is set to the ith member of the previous list
                maxTimeWaiting = timeWaiting
            if object.priority_giver:
                giver = object
                break
        return giver
    ...
    # =======================================================================
    def postProcessing(self, MaxSimtime=None, correction=None):
        from .Globals import G
        ...
        if self.isProcessing and not correction:
        ...
        activeObject.jobsDone.append(self.completedJobs)            # update the list with the number of jobs completed
        activeObject.SysTime.append(self.SysTimeList.tolist())      # update the list of the systime trace for every replication
        activeObject.IntArr.append(self.IntArrList.tolist())        # update the list of the inter arrival time for every replication

        if G.calculateFinalPosition:
            # append the parts in the object to
            for i in range(len(activeObjectQueue)):
                if activeObject.name == "Machine":
                    if activeObject.isBlocked:
                        if activeObject.next[0].name == "Transport":
                            G.FinalPositionList.append(int(activeObject.next[0].id[-3]))
                        else:
                            G.FinalPositionList.append(int(activeObject.next[0].id[-1]))
                    else:
                        G.FinalPositionList.append(int(activeObject.previous[0].id[-1]))
                elif activeObject.name == "Transport":
                    G.FinalPositionList.append(int(activeObject.next[0].id[-1]))
                else:
                    G.FinalPositionList.append(int(activeObject.id[-1]))
    ...
    def calculateProcessingTime(self):
        ...
        unloadTimeVal = self.unloadRng.generateNumber()  # calculate the unload time to add to the processing time
        if self.fromListFlag:
            return self.procTimeVal.obtainNumber() + unloadTimeVal
        else:       # this is if we have a default processing time for all the entities
            return self.rng.generateNumber() + unloadTimeVal
